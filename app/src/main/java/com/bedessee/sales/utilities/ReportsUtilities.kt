...
import com.bedessee.sales.store.WebViewer.Companion.show
...
class ReportsUtilities {
    companion object {
        ...
        fun openReportMenu(context: Context, reportsMenu: ReportsMenu, store: Store) {
            val file: File = FileUtilities.getFile(context, store.baseNumber, reportsMenu.popupType, reportsMenu.deviceFolder)
            if (file.exists()) {
                when (reportsMenu.popupType) {
                    ...
                    "HTM" -> {
                        show(context, FileUtilities.getFile(context, store.baseNumber, reportsMenu.popupType, reportsMenu.deviceFolder).absolutePath)
                    }
                }
            } else {
